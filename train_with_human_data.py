...
class TrainPipeline:
    ...
    def policy_updata(self):
        """更新策略价值网络"""
        mini_batch = random.sample(self.data_buffer, min(len(self.data_buffer), self.batch_size))
    ...
    def run(self):
        ...
        try:
            while True:
                try:
                    self.data_buffer = deque(maxlen=self.buffer_size) # 清空数据，避免污染
                    with open(CONFIG['train_data_with_human_buffer_path'], 'rb') as human_data_dict:
                        human_data_file = pickle.load(human_data_dict)
                        self.data_buffer.append(human_data_file['data_buffer'])
                        self.iters = human_data_file['iters']
                        del human_data_file
                    print('已载入ai与人类对弈的数据')
                    with open(CONFIG['train_data_buffer_path'], 'rb') as ai_data_dict:
                        ai_data_file = pickle.load(ai_data_dict)
                        self.data_buffer.append(ai_data_file['data_buffer'])
                        self.iters += ai_data_file['iters']
                        del ai_data_file
                    print('已载入ai自对弈的数据')
                    break
